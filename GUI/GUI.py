...
def connection_thread():
    ...
    while 1:
        ...
        if not car_info:
            ...
            all_btn_normal()
            ultrasonicMode = 0
            canvas_rec = canvas_ultra.create_rectangle(
                0, 0, 352, 30, fill=color_btn, width=0
            )
            canvas_text = canvas_ultra.create_text(
                (90, 11), text="Ultrasonic OFF", fill=color_text
            )
...
def instruction():
    ...
    while 1:
        ...
        instruction_4 = (
            "Q: Hand reaches out   E: Hand takes back   U & O: Hand rotation"
        )
            ...
            try:
                ...
                if float(ultra_data) < 3:
                    ...
                    canvas_rec = canvas_ultra.create_rectangle(
                        0,
                        0,
                        (352 - int(float(ultra_data) * 352 / 3)),
                        30,
                        fill="#448AFF",
                        width=0,
                    )
                    canvas_text = canvas_ultra.create_text(
                        (90, 11),
                        text="Ultrasonic Output: %sm" % ultra_data,
                        fill=color_text,
                    )
                    # print('xxx')
            except:
...
def socket_connect():  # Call this function to connect with the server
    ...
    for i in range(1, 6):  # Try 5 times if disconnected
        if ip_stu == 1:
            ...
            ultra_threading = thread.Thread(
                target=ultra_receive
            )  # Define a thread for FPV and OpenCV
...
def loop():  # GUI
    ...
    while True:
        ...
        try:
            logo = tk.PhotoImage(
                file="logo.png"
            )  # Define the picture of logo,but only supports '.png' and '.gif'
            l_logo = tk.Label(
                root, image=logo, bg=color_bg
            )  # Set a label to show the logo picture
            l_logo.place(x=30, y=13)  # Place the Label in a right position
        except:
            pass

        CPU_TEP_lab = tk.Label(
            root, width=18, text="CPU Temp:", fg=color_text, bg="#212121"
        )
        CPU_TEP_lab.place(x=400, y=15)  # Define a Label and put it in position

        CPU_USE_lab = tk.Label(
            root, width=18, text="CPU Usage:", fg=color_text, bg="#212121"
        )
        CPU_USE_lab.place(x=400, y=45)  # Define a Label and put it in position

        RAM_lab = tk.Label(
            root, width=18, text="RAM Usage:", fg=color_text, bg="#212121"
        )
        ...
        l_ip_4 = tk.Label(
            root, width=18, text="Disconnected", fg=color_text, bg="#F44336"
        )
        l_ip_4.place(x=400, y=110)  # Define a Label and put it in position

        l_ip_5 = tk.Label(
            root, width=18, text="Use default IP", fg=color_text, bg=color_btn
        )
        l_ip_5.place(x=400, y=145)  # Define a Label and put it in position

        label_ins = tk.Label(
            root, width=71, text="Instruction", fg=color_text, bg=color_btn
        )
        ...
        l_ip_3 = tk.Label(
            root, width=10, text="IP Address:", fg=color_text, bg="#000000"
        )
        l_ip_3.place(x=175, y=15)  # Define a Label and put it in position

        label_openCV = tk.Label(
            root, width=28, text="OpenCV Status", fg=color_text, bg=color_btn
        )
        label_openCV.place(x=180, y=110)  # Define a Label and put it in position

        canvas_ultra = tk.Canvas(
            root, bg=color_btn, height=23, width=352, highlightthickness=0
        )
        ...
        Btn0 = tk.Button(
            root, width=8, text="Forward", fg=color_text, bg=color_btn, relief="ridge"
        )
        Btn1 = tk.Button(
            root, width=8, text="Backward", fg=color_text, bg=color_btn, relief="ridge"
        )
        Btn2 = tk.Button(
            root, width=8, text="Left", fg=color_text, bg=color_btn, relief="ridge"
        )
        Btn3 = tk.Button(
            root, width=8, text="Right", fg=color_text, bg=color_btn, relief="ridge"
        )

        Btn_LeftSide = tk.Button(
            root, width=8, text="<--", fg=color_text, bg=color_btn, relief="ridge"
        )
        ...
        Btn_up = tk.Button(
            root, width=8, text="Up", fg=color_text, bg=color_btn, relief="ridge"
        )
        Btn_down = tk.Button(
            root, width=8, text="Down", fg=color_text, bg=color_btn, relief="ridge"
        )
        Btn_left = tk.Button(
            root, width=8, text="Grab", fg=color_text, bg=color_btn, relief="ridge"
        )
        Btn_right = tk.Button(
            root, width=8, text="Loose", fg=color_text, bg=color_btn, relief="ridge"
        )
        Btn_home = tk.Button(
            root, width=8, text="Home", fg=color_text, bg=color_btn, relief="ridge"
        )
        ...
        canvas_cover = tk.Canvas(
            root, bg=color_bg, height=30, width=510, highlightthickness=0
        )
        canvas_cover.place(x=30, y=420)

        Btn_Steady = tk.Button(
            root,
            width=10,
            text="Ultrasonic",
            fg=color_text,
        ...
        Btn_WatchDog = tk.Button(
            root, width=10, text="WatchDog", fg=color_text, bg=color_btn, relief="ridge"
        )
        ...
        Btn_Fun4 = tk.Button(
            root, width=10, text="FindLine", fg=color_text, bg=color_btn, relief="ridge"
        )
        ...
        ins_threading = thread.Thread(
            target=instruction
        )  # Define a thread for FPV and OpenCV
        ins_threading.setDaemon(
            True
        )  #'True' means it is a front thread,it would close when the mainloop() closes
