...
def loop():  # GUI
    ...
    while True:
        ...
        def connect(event):  # Call this function to connect with the server
            if ip_stu == 1:
                sc = thread.Thread(
                    target=socket_connect
                )  # Define a thread for connection
                sc.setDaemon(
                    True
        ...
        def code_receive():  # A function for data receiving
            ...
            while True:
                code_car = tcpClicSock.recv(
                    BUFSIZ
                )  # Listening,and save the data in 'code_car'
        ...
        l_ip_2 = tk.Label(
            root, width=18, text="pwm:%s" % (var_pwm.get()), fg=color_text, bg=color_btn
        )
        ...
        Btn0 = tk.Button(
            root, width=3, text="+", bd=0, fg=color_text, bg=color_btn, relief="ridge"
        )
        Btn1 = tk.Button(
            root, width=3, text="-", bd=0, fg=color_text, bg=color_btn, relief="ridge"
        )
...
if __name__ == "__main__":
    setPWM_threading = thread.Thread(
        target=set_pwm_thread
    )  # Define a thread for FPV and OpenCV
    setPWM_threading.setDaemon(
        True
    )  #'True' means it is a front thread,it would close when the mainloop() closes
